...
// 전체 시스템 백업 ZIP 파일을 클라이언트에게 다운로드 제공, 클라이언트가 업로드한 백업 파일로 시스템을 복원하는 엔드포인트 객체
@RestController
@RequestMapping("/api")
@CrossOrigin(origins = "*")
public class BackupApiController {
    ...
    // param: response - 백업 생성 과정에서 메모리에 만들어진 ZIP 스트림을 클라이언트에게 직접 전송(다운로드)하기 위한 HTTP
    @GetMapping("/backup")
    public void downloadBackup(HttpServletResponse response) throws IOException {
        // 응답 콘텐츠 타입을 압축 파일(ZIP) 형식으로 지정, 브라우저 인식
        response.setContentType("application/zip");
        // HTTP 헤더에 Content-Disposition을 설정, 다운로드될 파일명 지정
    ...
    // param: backupZipFile - 클라이언트가 멀티파트 요청(form-data)으로 업로드한 백업 ZIP 파일 객체
    @PostMapping("/restore")
